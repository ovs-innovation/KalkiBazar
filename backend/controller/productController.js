...
const getRecommendations = async (req, res) => {
  try {
    ...
    if (guestProductIds && Array.isArray(guestProductIds) && guestProductIds.length > 0) {
      const validGuestIds = guestProductIds.filter((id) => mongoose.Types.ObjectId.isValid(id));
      if (validGuestIds.length > 0) {
        const guestProds = await Product.find({
          _id: { $in: validGuestIds },
          status: "show",
    ...
    if (recommendations.length < 10) {
      ...
      let fillers = await Product.find({
        status: "show",
        _id: { $nin: excludeIds },
      })
      ...
      // If catalog is very small and we have fewer than 4 products total, allow viewed products to fill suggestions
      if (recommendations.length + fillers.length < 4) {
        const remainingLimit = 10 - (recommendations.length + fillers.length);
        const viewedFillers = await Product.find({
          status: "show",
          _id: { $in: viewedIds, $nin: recommendations.map(p => p._id) }
        }).limit(remainingLimit);
        fillers = [...fillers, ...viewedFillers];
      }
