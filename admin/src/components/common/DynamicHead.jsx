...
const DynamicHead = () => {
  ...
  useEffect(() => {
    ...
    const cleanName = (val, fallback) => {
      if (!val || val === "Ayurvedha Aaushdi" || val === "Farmacykart" || val.trim() === "") {
        return fallback;
      }
      return val;
    };
    ...
    const shopName = cleanName(globalSetting?.shop_name, "Kalki");
    const companyName = cleanName(globalSetting?.company_name, "Kalki");
    ...
    let metaTitle = storeCustomizationSetting?.seo?.meta_title;
    if (!metaTitle || metaTitle === "Ayurvedha Aaushdi" || metaTitle === "Farmacykart" || metaTitle.trim() === "") {
      metaTitle = "Kalki - Admin";
    }
